...
class image_converter:
  ...
  def save_predictions(self, img, preds):

    if time.time() - self.img_cnt < 0.5:
      return

    h, w, _ = img.shape
    print(h,w,_)

    name = 'test_env_' + str(rospy.Time.now().secs)
    img_path = './data/' + name + '.png'
    file_path = './labels/' + name + '.txt'

    with open(file_path, 'w') as f:
      for box in preds:
        xc = abs(box[2]+box[0])/2
        yc = abs(box[3]+box[1])/2
        boxH = abs(box[3]-box[1])
        ...
        label_str = "0 " + str(xc) + " " + str(yc) + " " + str(boxW) + " " + str(boxH) + "\n"
        f.write(label_str)
  ...
  def callback(self, rgb_image, depth_image):
    ...
    self.save_predictions(self.cv_image, bbox)
